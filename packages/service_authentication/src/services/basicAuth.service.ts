...
export const signup = async ({
    ...
    password,
    username,
}: models.classes.AuthUserDto) => {
    appLoggerService.verbose("Attempt to sign up new user", { email });
    if (await getUserByEmail(email)) {
        appLoggerService.info("attempt to sign up user failed - email taken", {
            email,
        });
    ...
    appLoggerService.verbose("User sign up completed successfully", {
        newUser,
    });
};
...
export const login = async ({
    ...
    username,
    password,
}: Partial<models.classes.AuthUserDto>) => {
    appLoggerService.verbose("Attempt to login a user", { email });
    if (!email && !username) {
        appLoggerService.warn(
            "Attempt for login failed - no email/username provided"
        );
        throw new errors.BadRequestError(
            "Must provide one of: email/username."
        );
    }
    if (!password) {
        appLoggerService.warn(
            "Attempt for login failed - no password provided"
        );
    ...
    if (!userFromDb) {
        appLoggerService.info(
            "attempt to login user failed - no user with email/username exists",
            { email, username }
        );
    ...
    appLoggerService.verbose(
        "user password was correct - attempting to build user info",
        {
            userFromDb,
        }
    );
...
export const requestPasswordReset = async ({
    email,
}: models.classes.RequestPasswordResetDto) => {
    appLoggerService.verbose(
        "Attempt to request a password reset token for user.",
        { email }
    );
    //check if user exist
    ...
    if (!userFromDb) {
        appLoggerService.info(
            "attempt to create reset token has failed - no user has this email",
            {
                email,
            }
        );
    ...
    try {
        ...
        appLoggerService.verbose("created token for password reset", {
            token,
            email,
        });
        ...
        appLoggerService.verbose(
            "sent token for password reset to user's email",
            { token, email }
        );
    } catch (err) {
...
export const resetPassword = async (
    token: string,
    { password }: models.classes.ResetPasswordDto
) => {
    appLoggerService.verbose(
        "Attempt to request a password reset with token.",
        { token }
    );
    //check if user exist
    ...
    if (!userFromDb) {
        appLoggerService.info(
            "attempt to reset password has failed - no user has this token",
            {
                token,
            }
        );
    ...
    appLoggerService.verbose("Found user with given reset token.", {
        userFromDb,
    });
    ...
    appLoggerService.verbose("User with new password saved.", {
        userFromDb,
        hashedPassword,
    });
};
